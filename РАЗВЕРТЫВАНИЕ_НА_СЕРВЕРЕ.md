...
## ⚠️ ВАЖНО: Использование IPv4

**Все команды в этой инструкции используют IPv4 адрес сервера.**

Замените `YOUR_IPv4_ADDRESS` на ваш реальный IPv4 адрес сервера (например, `192.168.1.100`).
...
```bash
# 1. Подключитесь к серверу по SSH (используйте IPv4 адрес)
...
# Скопируйте публичный ключ на сервер (замените YOUR_IPv4_ADDRESS):
type $env:USERPROFILE\.ssh\id_ed25519.pub | ssh botuser@YOUR_IPv4_ADDRESS "mkdir -p ~/.ssh && cat >> ~/.ssh/authorized_keys"
# Пример: type $env:USERPROFILE\.ssh\id_ed25519.pub | ssh botuser@192.168.1.100 "mkdir -p ~/.ssh && cat >> ~/.ssh/authorized_keys"
...
```bash
# Подключитесь как botuser (используйте IPv4 адрес)
...
**ВАЖНО:** Подключитесь к серверу по SSH перед выполнением команд:

```bash
# 1. Подключитесь к серверу (используйте IPv4 адрес)
ssh botuser@YOUR_IPv4_ADDRESS
# Пример: ssh botuser@192.168.1.100
...
**Если репозиторий приватный, настройте авторизацию:**
- Используйте Personal Access Token (см. ПОДКЛЮЧЕНИЕ_GIT_К_СЕРВЕРУ.md)
...
```bash
# Через WSL (используйте IPv4 адрес)
...
# Пример: botuser@192.168.1.100:~/trading_bot/
```
...
**Или скопируйте через SCP (используйте IPv4 адрес):**
...
**Вариант 2: Вручную**
```bash
# На сервере (используйте IPv4 адрес)
ssh botuser@YOUR_IPv4_ADDRESS
# Пример: ssh botuser@192.168.1.100
cd ~/trading_bot
./update.sh
